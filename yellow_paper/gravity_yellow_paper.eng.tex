...
Gravity blockchain is integrated with social network. Every account may have social relations with any other account and may have some content belonging to it. Accounts and content objects with relations between them form a graph. Every account and content object obtains rate depending on involving into social relations. We denote this rate as social network activity index. It is contrubute to gravity index.
...
Further, outlink matrices can be calculated with normalization every column to 1:

$$
o_{XYij} = \begin{cases}
 \frac{w_{XYij}} {\sum\limits_{k} w_{XYkj}}
 & \text{if $\sum\limits_{k} w_{XYkj}> 0$,}\\
 0 & \text{otherwise}
\end{cases} 
$$
